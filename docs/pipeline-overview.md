...
- **Stacked PR base on `origin`**: Before rebase and PR, the pipeline checks that `origin/<stack-base>` exists after `git fetch`. If the parent stacked branch was never pushed (e.g. an earlier subtask skipped PR or failed before push), push that branch or merge its PR first — otherwise rebase and `gh pr create --base` cannot succeed.
