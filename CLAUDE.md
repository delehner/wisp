...
wisp generate prd                                # Interactively generate a PRD
wisp generate context --repo https://github.com/org/repo
wisp monitor --workdir /path                     # Tail agent logs in real-time
...
Log level is set via `RUST_LOG` env var (default: `info`).
...
trait Provider {
    ...
    fn build_run_args(&self, opts: &RunOpts) -> Vec<String>;  // build CLI args
    fn extract_session_id(&self, line: &str) -> Option<String>; // parse JSONL
    fn resume_hint(&self, session_id: &str) -> String;
}
```

- `create_provider(kind: ProviderKind) -> Box<dyn Provider>` is the factory function
...
Configuration is loaded by `src/config.rs` via `dotenvy` from a `.env` file in the project root. All variables can also be set as real environment variables.

## Provider Selection

```env
WISP_PROVIDER=claude        # or: gemini (default: claude)
```
...
```env
WISP_CLAUDE_MODEL=claude-sonnet-4-6          # default Claude model
WISP_GEMINI_MODEL=gemini-2.5-pro             # default Gemini model
```
...
```env
WISP_ARCHITECT_MODEL=claude-opus-4-6
WISP_DEVELOPER_MODEL=claude-sonnet-4-6
WISP_TESTER_MODEL=claude-haiku-4-5-20251001
...
```env
WISP_MAX_PARALLEL=3               # max concurrent PRD pipelines
WISP_MAX_ITERATIONS=10            # default Ralph Loop iterations per agent
WISP_BASE_BRANCH=main             # default base branch
WISP_WORK_DIR=/tmp/wisp-work      # directory for cloned repos
WISP_CLEANUP=true                 # remove workdirs after pipeline
WISP_USE_DEVCONTAINER=true        # run agents inside dev containers
WISP_SKIP_PR=false                # dry-run mode (no PR creation)
WISP_INTERACTIVE=false            # pause between agents for review
```

### Per-Agent Max Iteration Overrides

```env
WISP_ARCHITECT_MAX_ITERATIONS=5
WISP_DEVELOPER_MAX_ITERATIONS=15
...
```env
WISP_EVIDENCE_AGENTS=tester,secops,performance   # agents whose reports become PR comments
```
...
WISP_LOG_DIR=/tmp/wisp-logs       # JSONL log output directory
WISP_VERBOSE=false                # enable verbose CLI output
```
...
- Session resumption: extract `session_id` from first 5 JSONL lines; use `--resume <id>` to continue

### Gemini CLI (`gemini`)
- Installed separately; checked via `which gemini`
- Invoked as: `gemini -p <prompt-file> --model <model> --yolo --output-format stream-json`
- Session resumption: extract `sessionId` or `session_id` from first 10 JSONL lines
